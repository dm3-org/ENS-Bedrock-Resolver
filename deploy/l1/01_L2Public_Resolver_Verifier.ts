...
async function main() {
    ...
    console.log({
        ownerAddress:owner.address, GRAPHQL_URL,CHAIN_NAME, CHAIN_ID, BEDROCK_PROOF_VERIFIER_ADDRESS, L2_RESOLVER_ADDRESS
    })     
    const deployTx = await L2PublicResolverVerifier
        .deploy(
            owner.address, GRAPHQL_URL, CHAIN_NAME, CHAIN_ID,BEDROCK_PROOF_VERIFIER_ADDRESS, L2_RESOLVER_ADDRESS
            ,{gasLimit: 5000000}
        );
    ...
    console.log(
            `Verify the contract using  npx hardhat verify --network ${hre.network.name} ${deployTx.address} ${owner.address} ${GRAPHQL_URL} "${CHAIN_NAME}" ${CHAIN_ID} ${BEDROCK_PROOF_VERIFIER_ADDRESS} ${L2_RESOLVER_ADDRESS} `
    );

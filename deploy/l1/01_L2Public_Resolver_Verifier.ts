...
async function main() {
    ...
    const deployTx = await L2PublicResolverVerifier.deploy(BEDROCK_PROOF_VERIFIER_ADDRESS, L2_PUBLIC_RESOLVER_ADDRESS);
    await deployTx.deployed();

    console.log(`L2PublicResolverVerifier deployed at  ${deployTx.address}`);
    console.log(
        `Verify the contract using  npx hardhat verify --network ${hre.network.name} ${deployTx.address} ${BEDROCK_PROOF_VERIFIER_ADDRESS} ${L2_PUBLIC_RESOLVER_ADDRESS} `
    );
}
